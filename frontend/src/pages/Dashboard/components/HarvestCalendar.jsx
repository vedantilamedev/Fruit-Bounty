            ...
            <div className="flex items-center justify-end mb-4 px-2">
                <div className="flex items-center gap-3">
                    ...
                    <div className="px-6 py-3 bg-white border-2 border-[#E8E4D9] rounded-2xl shadow-sm min-w-[140px] text-center">
                        <span className="font-bold text-[#2f6131] text-sm uppercase tracking-widest italic">{monthNames[currentDate.getMonth()]} {currentDate.getFullYear()}</span>
                    </div>
                    <button
                        onClick={() => {
                            const newDate = new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 1);
    ...
    const renderDaysOfWeek = () => {
        ...
        return (
            <div className="grid grid-cols-7 mb-4 px-2 lg:px-3 gap-1 lg:gap-3">
                {days.map(day => (
                    <div key={day} className="text-center text-[9px] lg:text-[10px] font-medium text-[#B7A261] uppercase tracking-wider italic mb-2">
                        {day}
                    </div>
                ))}
            </div>
            ...
            cells.push(
                <motion.div
                    ...
                    className={`h-20 lg:h-32 border p-2 lg:p-3 cursor-pointer relative transition-all duration-300 group rounded-2xl
                        ${isSelected
                            ? 'bg-white border-[#3e7b3f] border-2 ring-4 ring-[#3e7b3f]/10 z-10 shadow-xl'
                            : 'bg-white border-[#E8E4D9]/40 hover:bg-[#FBF8F2] hover:border-[#3e7b3f]/30'}
                    `}
                >
                    <span className={`text-[10px] lg:text-xs font-bold tracking-tighter ${isSelected ? 'text-[#3e7b3f]' : 'text-[#6B705C] group-hover:text-[#2f6131]'} ${isToday ? 'bg-[#B7A261] text-white px-2 py-0.5 rounded-md font-black' : ''}`}>
                    ...
                    {event && (
                        <div className="mt-1 lg:mt-2 flex flex-col gap-1">
                            <motion.div
                                ...
                                className={`px-1.5 py-0.5 lg:px-2 lg:py-1 rounded-lg text-[8px] lg:text-[9px] font-bold uppercase tracking-tighter flex items-center gap-1 shadow-sm
                    ...
                    {isSelected && (
                        <motion.div
                            ...
                            className="absolute bottom-2 right-2 w-1.5 h-1.5 lg:w-2 lg:h-2 rounded-full bg-[#3e7b3f]"
                        />
                    )}
                </motion.div>
            );
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
                <div className="lg:col-span-2 bg-white p-8 lg:p-10 rounded-[3rem] border-2 border-[#E8E4D9] shadow-sm relative overflow-hidden group">
                    ...
                    <div className="relative">
                        <p className="text-[#B7A261] font-bold text-[10px] uppercase tracking-widest italic mb-2">Selected Date Detail</p>
                        ...
                        {event ? (
                            <AnimatePresence mode="wait">
                                <motion.div
                                    ...
                                    className="space-y-6"
                                >
                                    <div className={`p-6 lg:p-8 rounded-[2rem] shadow-sm border ${event.type === 'harvest' ? 'bg-[#b7a261]/5 border-[#b7a261]/10' : 'bg-[#3e7b3f]/5 border-[#3e7b3f]/10'}`}>
                                        <div className="flex items-center gap-4 mb-6">
                                            <div className={`p-3 lg:p-4 rounded-2xl ${event.type === 'harvest' ? 'bg-[#b7a261]' : 'bg-[#3e7b3f]'} text-white shadow-lg shadow-current/20`}>
                                                {event.type === 'harvest' ? <Leaf size={28} strokeWidth={2.5} /> : <Truck size={28} strokeWidth={2.5} />}
                                            </div>
                                            <div>
                                                <h4 className={`text-2xl font-black italic tracking-tight ${event.type === 'harvest' ? 'text-[#b7a261]' : 'text-[#3e7b3f]'}`}>{event.title}</h4>
                                        ...
                                        <div className="space-y-4">
                                            <p className="text-base font-normal text-[#6B705C] leading-relaxed">
                                                {event.desc || `Your organic bounty is scheduled for arrival on this day.`}
                                            </p>
                                            <div className="flex flex-wrap gap-2 pt-2">
                                                {event.items.map(item => (
                                                    <span key={item} className="px-5 py-2 bg-white border border-[#E8E4D9] rounded-full text-[11px] font-bold text-[#6B705C] uppercase tracking-tighter shadow-sm hover:border-[#3e7b3f]/30 transition-colors">
                                                        {item}
                                                    </span>
                                                ))}
                                            </div>
                                    ...
                                    <button className={`w-full py-5 rounded-full font-black text-xs uppercase tracking-[0.2em] transition-all duration-300 shadow-xl
                                        ${event.type === 'harvest' ? 'bg-[#b7a261] hover:bg-[#a49156] shadow-orange-900/10' : 'bg-[#3e7b3f] hover:bg-[#2f6131] shadow-green-900/10'} text-white hover:scale-[1.01]
                                    `}>
                            ...
                            <div className="py-16 text-center flex flex-col items-center border-2 border-dashed border-[#E8E4D9] rounded-[2.5rem] bg-[#FBF8F2]/50">
                                ...
                                <p className="text-[#2f6131] font-black text-xl italic tracking-tight opacity-60 px-4">No events on this tranquil day</p>
                                <p className="text-[#6B705C] font-normal text-xs mt-2 uppercase tracking-widest">Select a marked date to see more details</p>
                            </div>
                        )}
                    </div>
                </div>

                <div className="lg:col-span-1 bg-[#2f6131] p-8 lg:p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden shadow-emerald-900/20 flex flex-col justify-center">
                    ...
                    <div className="relative flex flex-col items-start gap-6">
                        ...
                        <div>
                            <h5 className="font-black text-2xl italic mb-3 tracking-tight uppercase">Waitlist Notice</h5>
                            <p className="text-white/70 text-sm font-normal leading-relaxed tracking-wide uppercase">
                                Special seasonal harvests can be pre-booked 7 days in advance. Keep an eye on the <span className="text-[#B7A261] font-bold">Gold Markers</span>!
                            </p>
                        </div>
                    </div>
                </div>
