...
const Payments = ({ payments }) => {
    ...
    const statusStyles = {
        Success: 'bg-[#3C7E44]/10 text-[#3C7E44] border-[#3C7E44]/20',
        ...
        <div className="space-y-8 animate-fadeIn mt-4">
            ...
            {payments.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-20 bg-white rounded-[3rem] border-2 border-dashed border-[#E8E4D9] shadow-inner text-center">
                    ...
                    <p className="text-black font-normal text-xl mb-2 tracking-tight">Financial Records Clear!</p>
                    <p className="text-black font-medium text-sm max-w-xs">You haven't made any fruit investments yet. Your vault is ready when you are.</p>
                </div>
            ) : (
                <div className="bg-white rounded-[3rem] shadow-sm border border-[#E8E4D9] overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-[#F7F5EF] text-[10px] text-[#B7A261] uppercase font-normal tracking-[0.15em] border-b border-[#E8E4D9]">
                            ...
                            <tbody className="divide-y divide-[#E8E4D9]/30 text-sm">
                                {payments.map((payment) => (
                                    <tr key={payment.id} className="hover:bg-[#FBF8F2] transition duration-300 group">
                                        <td className="p-6 font-normal text-black text-xs">#{payment.id}</td>
                                        <td className="p-6 text-black font-normal opacity-80">{payment.date}</td>
                                        <td className="p-6 font-normal text-[#3C7E44] text-lg">₹{payment.amount}</td>
                                        <td className="p-6 font-normal text-[#B7A261] uppercase text-[10px] tracking-widest">{payment.method}</td>
                                        <td className="p-6">
                                            <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-[10px] font-normal uppercase tracking-widest border ${statusStyles[payment.status] || 'bg-gray-50 text-gray-600'}`}>
