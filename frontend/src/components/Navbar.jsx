import { Store, ShoppingCart, User, LogIn } from "lucide-react"; // Changed ShoppingBag to Store
      ...
      <nav className="w-full bg-gradient-to-r from-[#5a8c4f] via-[#4d8a43] to-[#8eb58a]">
        <div className="max-w-6xl mx-auto flex justify-between items-center px-6 py-4">
          ...
          <div className="flex items-center gap-10">
            <Link to="/">
              {/* Change: Increased Logo Size to h-12 */}
              <img src="/images/footerlogo.webp" alt="Logo" className="h-12 object-contain cursor-pointer" />
            </Link>

            <div className="flex gap-8 text-white font-medium">
              <div className="relative py-1">
                <Link to="/" className="transition-colors duration-200 hover:text-[#C9C27A]" style={{ color: location.pathname === "/" ? "#C9C27A" : "white" }}>Home</Link>
                {location.pathname === "/" && <div className="absolute -bottom-1 left-0 w-full h-0.5 bg-[#C9C27A] rounded-full"></div>}
              </div>

              <div className="relative py-1">
                <Link to="/customize" className="transition-colors duration-200 hover:text-[#C9C27A]" style={{ color: location.pathname === "/customize" ? "#C9C27A" : "white" }}>Customize your bowl</Link>
                {location.pathname === "/customize" && <div className="absolute -bottom-1 left-0 w-full h-0.5 bg-[#C9C27A] rounded-full"></div>}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/cart">
              <button className="w-10 h-10 bg-green-900 rounded-full flex items-center justify-center transition-colors duration-300 hover:bg-[#C9C27A] group relative">
                ...
                {cart.length > 0 && <span className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] px-1.5 py-0.5 rounded-full font-bold">{cart.length}</span>}
              </button>
            </Link>

            <div className="relative" ref={authDropdownRef}>
              <button onClick={() => setAuthOpen(!authOpen)} className="w-10 h-10 bg-green-900 rounded-full flex items-center justify-center transition-colors duration-300 hover:bg-[#C9C27A] group focus:outline-none">
                ...
                <div className="absolute right-0 mt-3 w-48 bg-white rounded-xl shadow-xl overflow-hidden z-50">
                  {!token ? (
                    ...
                    <div className="p-2 space-y-1">
                      ...
                      <button onClick={() => { localStorage.removeItem("token"); window.location.href = "/"; }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-red-600 hover:bg-red-50 rounded-lg transition">Logout</button>
                    </div>
                  )}
                </div>
              )}
